...
def reordenamiento_excel1(data1):# funcion para procesar el primer csv ya que las columnas a procesar estan en un orden distinto
    ...
    for aux in data1:# for que procesa archivo 1
        ...
        if aux[2] not in fecha:
            paises[aux[0]].append({"fecha": aux[2], "tipo vacuna" : aux[12], "vacunados a la fecha" : float(aux[4]),"personas completamente vacunadas" : float(aux[5])})
...
def reordenamiento_excel2(data2,diccionario):# funcion que procesa el csv 2 y agrega al diccionario 
    ...
    '''
    # funcion para buscar dentro de lo que se te ocuura
    # esta en especìfico es para buccar  y sumar los vacunas
    # de austra el dìa 2021 01 05
    variable = 0
    for i in diccionario.get("Paises").get("Austria"):
        #print(i)
        if i.get("fecha") == "2021-01-05":
            print(i.get("vacunados a la fecha"))
            variable += i.get("vacunados a la fecha")
...
def main():
    ...
    with open ("diccionario.json", "w") as file:# crear archivo json 
        ...
        #ejemplo2= json.dump(diccionario2,file,indent=4)#agrego al json diccionario 2
        file.write(ejemplo1)#escribo el archivo 1 
        #file.write(ejemplo2)#escribo el archivo 2 luego de haber procesado el archivo 1
